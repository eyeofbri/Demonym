...
You are in possession of a Habitat.

At first, there may not appear to be much inside it.

Wait.

The signal is still forming.

A **Demonym** is not quite a pet, not quite a program, and not quite something that should be alive. It develops inside the Cardputer, responding to care, training, conflict, exploration, movement, and whatever survives from the generations that came before it.

You are its **Keeper**.

Your job is not to force the organism toward one perfect build. Your job is to keep it alive long enough to become something of its own.

A successful life may produce a powerful battler. It may produce a strange traveler, a resilient scavenger, a recognizable rival, or an ancestor whose traits remain visible generations later.

There is no single correct Demonym.

There are only the ones that made it this far.

> **KEEPER TIP**  
> Demonym is built around accumulation. A training score, a discovered Fragment, a scar, a rival, a Habitat object, or a strange inherited trait may matter long after the moment that produced it.
...
Every active organism has several layers of identity.
...
Two Keepers can begin with similar organisms and still end up with creatures that look, behave, fight, and develop differently.

### A Demonym is persistent

Demonym is designed around one active creature at a time. The organism does not reset when you close a menu, lose a battle, or turn the device off.
...
On a fresh installation, Demonym creates an organism and begins at the **Egg** stage.

The Egg is not an idle loading screen. It is the first interaction with the organism.
...
The early game is intentionally small. Feed it. Rest it. Observe it. Try Training. Learn how quickly Energy disappears. Open Stats. Listen to its cry. Let it spend some time alive before trying to optimize everything.
...
Demonym is designed to be played entirely from the Cardputer keyboard.
...
Battle presents contextual command buttons and move lists. Use the directional controls to change selection and **Enter** to commit.
...
The exact arrangement is intentionally compact. More detailed numbers and history live in **Stats**.

### Observation

Press **Enter** on the Habitat when no other action is active to observe or interact with the creature.

The response is brief. These messages are not meant to explain every hidden system. They are small clues about condition, temperament, development, or current behavior.
...
> **FIELD NOTE**  
> A quiet Habitat is not an empty system. The organism continues accumulating active-time history even when nothing dramatic is happening on-screen.
...
The game may describe development in broad states rather than exposing the complete formula. You are meant to understand the direction of a life without seeing every hidden weight.
...
It means the creature has finally become capable of leaving a meaningful record behind.
...
Healing items and sleep can restore Health, but recovery is limited by Hunger. A badly fed organism cannot recover to full condition simply because you have enough medicine.
...
Hunger also limits maximum recoverable Health. This makes feeding part of medical care rather than a separate cosmetic meter.
...
Most direct care is handled through **Inventory** and **Rest** rather than a separate “Care” menu.
...
The **Scoop** is a reusable care tool sold by the Shop. Once owned, it stays with the Habitat rather than behaving like a normal consumable item.
...
> **FIELD NOTE**  
> The organism appears embarrassed by none of this.
...
Work is a low-risk way to earn Coins through short signal-processing jobs rather than combat or exploration.
...
Juvenile development is influenced by a mixture of care and behavior. Demonym intentionally does not expose every hidden score.

Watch broad development states, changes in behavior, Training history, Stress, and the creature's reactions.

If the organism is healthy, active, interacting, and Training, development will eventually move forward.

> **KEEPER TIP**  
> Do not try to solve evolution from one number. Demonym is designed so that a life pattern matters more than a single perfect action.
...
> **KEEPER TIP**  
> A Training program is allowed to be bad for the creature if you play it badly. The score screen is not the only result screen that matters—check the Habitat afterward.
...
Tasks are deliberately small: packet handling, sorting, timing, and other compact signal jobs.

Relay Work pays Coins, but repeated work is limited by cooldowns and diminishing opportunity. It is meant to help support care and expedition costs, not replace every other system.
...
Some Crossings contain hazards or special events. Heat-related Crossing events are deliberately uncommon rather than constant.
...
The field is not meant to be solved by rapidly selecting every tile. Energy and Stability make information valuable.
...
Retreat attempts to leave a normal eligible battle.

Retreat can fail.

Repeated attempts therefore consume turns and expose the creature to further danger.
...
Roam depth and stronger encounters generally produce more meaningful progression than endlessly fighting trivial opponents.

## Injury

Battle consequences are not always erased when the result screen appears.
...
Duplicate discoveries may produce a smaller material reward rather than pretending the same discovery is new.

## Adaptations

Adaptations are more substantial modifications derived from the creature's development and discoveries.
...
Adaptations are deliberately restrained. One discovered trait should not erase the creature's primary Lineage.
...
Compatibility with a future organism may be described as:
...
When the other Keeper is still choosing, the game waits rather than guessing their move.
...
This can include its identity, Lineage/form, Level, and other intentionally shareable battle information.
...
Chat is deliberately tiny. It is not a messaging platform.
...
Signalpedia history survives the replacement of the active creature. It belongs to the Keeper's wider installation history rather than only one organism.
...
A Lineage is not a class chosen from a character-select screen. It is the organism's biological/signal family.
...
The manual intentionally does not list every form or exact developmental recipe.

You are raising an organism, not ordering one.
...
The next creature remains its own organism.
...
When a legitimate Adult generation reaches its end, Demonym can create an **Echo Egg**.

Before the next life begins, the Keeper can review eligible ancestral Legacy information and choose what kind of echo should be bound to the new generation.
...
A creature lost through severe failure or runaway conditions should not necessarily provide the same inheritance as one that completed a legitimate life.

Signal Legacy is a reward for stewardship and history, not a universal reset bonus.
...
The Cardputer contains a motion sensor, and Demonym can use broad movement history as part of the organism's environmental record.

This system is intentionally modest.
...
Ordinary power-off freezes the simulation rather than punishing the Keeper for not leaving the Cardputer running all night.
...
A new entry may explain something that the pickup screen intentionally did not.

### 14. Play against the same Keeper more than once

The Rival Book becomes more meaningful when an opponent stops being anonymous.
...
**Adaptation**  
A meaningful creature modification that can affect appearance, stats, Pressure behavior, or passives.
...
**Demonym**  
The organism being raised; also the name of the game/firmware.

**Depth**  
A deeper procedural Roam layer reached by descending after completing the current one.

**Echo Egg**  
A new-generation Egg created after a legitimate Legacy transition.
...
**Habitat Object**  
A recovered persistent object that can be displayed or interacted with in the Habitat rather than consumed normally.
...
> Several appear to have been deliberately placed before recovery.
